// You have an array of integers, and 
// for each index you want to find the 
// product of every integer except the 
// integer at that index. You may not 
// use division.

function getProductsOfAllIntsExceptAtIndex(intArray) {
  if(intArray.length < 1) {throw new Error('emptyArray')};
  let sum = [];
  
  for(let i = 0; i < intArray.length; i++) {
      const reducer = (accumulator, currentValue) => accumulator * currentValue;
      let dropIntArr = [...intArray];
      dropIntArr.splice(i, 1);
      sum[i] = dropIntArr.reduce(reducer);
  }
